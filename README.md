...
| **抚摸** | 按住左键即抚摸：**GPU 像素级 alpha 命中检测**（贴合真实轮廓，不是矩形框），摸到头部冒爱心、眯眼、说台词、发出轻柔的抚摸音效；被摸时眼睛停住但**头部仍轻微跟随鼠标**；**无好感度数值，界面不显示任何 HUD** | 注入真实鼠标事件按住并来回抚摸，情绪切换为 `happy`；眯眼/头部跟随/音效开关逐条断言通过 |
...
| **设置面板** | VS Code 风格：分类树 + 搜索 + 键名显示 + 修改标记；共 114 个设置项，参数**支持锁定** | 锁定语义、参数上下限、开关落实、抚摸设置断言通过 |
...
| 操作 | 效果 |
| --- | --- |
| **按住左键在模型上移动** | **抚摸** —— 摸到头部时冒爱心、眯眼（可关）、发出抚摸音效、随机台词 |
...
| [优化建议](docs/优化建议.md) | 暂不发布（文件内容即为占位说明） |
